...
export function ColabSections({ data }: Props): React.ReactElement {
  ...
  const navigate = useCallback(
    (direction: 1 | -1) => {
      ...
      setActiveIndex(next);
      setTimeout(() => { canNavigateRef.current = true; }, 700);
    },
    [count],
  );
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 30) navigate(dx > 0 ? 1 : -1);
    };
  ...
  return (
    <>
      ...
      <div className="lg:hidden flex flex-col">
        ...
        <YouMightBeInterested title="More" />
      </div>
        ...
        <AnimatePresence>
          {activeIndex === 0 && (
            <motion.button
              ...
              scroll
              <svg width="35" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <AnimatePresence>
          {activeIndex > 0 && (
            <motion.div
              ...
              <RdProgressBar
                sections={SECTIONS}
                activeId={SECTIONS[activeIndex]?.id ?? "intro"}

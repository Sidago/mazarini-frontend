...
import { NewsFilterCard } from "@/components/news/news-filter-card";
...
export function NewsGrid({ news }: NewsGridProps): React.ReactElement {
  ...
  const categories = useMemo(
    () =>
      Array.from(
        new Set(
          news.map((n) => n.category).filter((c): c is string => c !== null),
        ),
      ).sort(),
    [news],
  );
